...
CASTING_COLUMNS = [
    "資料結構版本", "系統版本", "排卦ID", "建立時間",
    "event_at", "freeze_at", "cast_at", "時間基準版本", "樣本分類",
    "事件時間來源等級", "事件時間來源網址", "起卦國曆ISO", "起卦農曆時間",
    "起卦時區", "實際執行時區", "農曆年份", "農曆年干支", "農曆月份", "是否閏月", "農曆日",
...
def build_casting_row(
    ...
    return {
        ...
        "event_at": result.event_moment.gregorian_iso,
        "freeze_at": result.freeze_at_iso,
        "cast_at": result.casting_moment.gregorian_iso,
        "時間基準版本": "event-at-only-v1",
        "樣本分類": casting.sample_class,
        "事件時間來源等級": casting.event_source_grade,
        "事件時間來源網址": casting.event_source_url,
        "起卦國曆ISO": result.event_moment.gregorian_iso,
        "起卦農曆時間": result.event_moment.lunar_text,
        "起卦時區": f"{result.event_moment.timezone}／{result.event_moment.utc_offset}",
        "實際執行時區": f"{result.casting_moment.timezone}／{result.casting_moment.utc_offset}",
        "農曆年份": result.event_moment.lunar_year,
        "農曆年干支": result.event_moment.lunar_year_ganzhi,
        "農曆月份": f"{result.event_moment.lunar_month_text}月",
        "是否閏月": "是" if result.event_moment.lunar_is_leap_month else "否",
        "農曆日": result.event_moment.lunar_day_text,
        "起卦時辰": f"{result.event_moment.shichen}時",
        "起卦干支時辰": f"{result.event_moment.shichen_ganzhi}時",
