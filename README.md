...
**JARVIS 10.2 · Deep Divination Review** 把產品固定在一條可稽核工作流：JARVIS 負責事件時間、奇門起局、梅花起卦、古籍／知識檢索、來源審查、矛盾與不確定性整理；最後的術數綜合判讀交給 ChatGPT。
...
```text
MATCH_EVENT_V1
同一個事件所在地 aware local datetime + IANA timezone
        ↓
┌───────────────────────┬────────────────────────┐
...
網站新增 **「⚽ 足球 Case」**：一次輸入主客隊、賽事／階段／球場、事件時間與 IANA timezone，同時建立奇門與梅花 packet。兩份 packet 只有在主客、事件時間與 timezone 完全一致時才能組成 Case Bundle。
...
Runtime 使用 `streamlit==1.61.0`、`lunar_python==1.4.8`、`tzdata==2026.3`。

- 事件時間支援秒級輸入。
- 使用 IANA timezone，而不是只存 EST / EDT / CDT 等縮寫。
- DST 回撥的 ambiguous local time 必須明確選 `fold=0` 或 `fold=1`；DST 跳時造成的 nonexistent time 直接拒絕。
- 梅花足球預設 180 分鐘 wall-clock audit，可選 120 / 150 / 180 / 210。
...
**時家奇門・轉盤・拆補法・事件所在地民用時・晚子時換日・中五寄坤二・天禽隨天芮・八神同名制。**
...
- 80 十天干 × 八門
- 90 十天干 × 九星
- 旬空、驛馬、伏吟、反吟、門迫、入墓、擊刑等 modifier 於實際盤面動態疊加

共 **378 個靜態 Extended Relations**，authority 固定為 `PROJECT_HEURISTIC__COMPONENTS_SOURCE_BACKED`。這些組合不是古籍逐條專名，也不得做符號投票、勝率或固定比分。

### Qimen source-derived golden fixtures

10.2 新增四個可重建 classical method fixtures，來源錨定《遁甲演義》卷二，並用《奇門遁甲統宗》轉布規則人工固定 full core expected plate：
...
CI 逐宮核對地盤、天盤干、星、門、值符／值使與 source anchor。古籍例沒有完整西曆年份／IANA timezone，且八神名制存在流派差異，因此這些 fixture 被明確稱為 **`SOURCE_DERIVED_METHOD_GOLDEN`**，不冒充完整 calendar + timezone + deity 的 end-to-end external certification。

## 梅花易數古法方法審查

目前 production engine 只實作 **年月日時起卦**，分類為 `XIANTIAN_NUMBER_METHOD`。

傳世《梅花易數》方法審查被正式寫入 packet：

- 先天數法：目前年月日時屬此類；體用、旺衰、互變、動靜與內外是主要骨架，《周易》文本為 `SUPPORTING`。
...
- 閏月與日界 convention 顯式版本化，不再隱藏在算法裡。
...
`cross_system_coherence` 只在來源 pair 對齊後比較周易動爻與易林條目的 project semantic domains；共同 domain 是條件式候選 reinforcement，不是吉凶投票。
...
保存同一場足球的 Qimen + Meihua packets，驗證：

- same home / away
- same aware event datetime
- same IANA timezone
- packet SHA integrity
- deterministic match-event identity
...
- `pages/3_Knowledge_Vault.py` — 知識庫
- `pages/4_AI_Packet.py` — 單一 AI Packet
- `pages/5_Football_Case.py` — 足球雙術數 Case Bundle
...
JARVIS 10.2 完成的是：pinned Zhouyi / Yilin source review、384 爻條件式 review、Meihua method-aware / temporal review、Qimen Core 306 + Extended 378、source-derived Qimen method golden tests，以及 deterministic Case Bundle handoff。
