# 梅花易數足球 AI 自主推理系統 v4.2

這是一個以 Streamlit、GitHub Contents 與 GitHub Models 建立的足球賽前研究系統。v4.2 不再先把卦象壓成分數再讓 AI 補理由：它先保存完整語義卦線，讓第一階段 AI 在看不到比分、機率、λ、實力分與相似案例賽果時盲解主線與反線；第二階段才用足球先驗校準並選比分。
...
4. 純足球先驗先建立雙方期望進球 λ。
5. 體、用、本卦、互卦、動爻、體用轉象、變卦與五行生剋必須先形成一條有時序、有主解也有反解的語義卦線。
...
11. AI 第一階段不接收比分候選、Poisson 網格、λ、實力分、量化維度、規則排序或相似案例實際比分。
12. AI 第二階段只接收第一階段盲解、足球先驗、有界調整與中性排序候選；若不同意盲解，必須提出具體反證。
13. 本地語義骨架與大型語言模型盲解必須清楚區分；AI 未啟用時不得把模板宣稱為 AI 思考。
14. 足球基線、有界 λ、本地語義、AI 盲解、AI 決策與最終排序分開保存與評估。

## 預測流程

```mermaid
flowchart TD
    A[賽前文字] --> B[固定起卦與本地語義卦線]
    B --> C[第一階段 AI 盲解]
    D[獨立足球先驗] --> E[第二階段足球校準]
    C --> E
    E --> F[中性候選池內三比分]
...
- 數值前先顯示主局、中段、動爻、終局、主解、反解、兩方破門路徑與成立／失效條件。
- GitHub Models 兩階段推理：盲解卦與足球比分決策使用兩個隔離的 Prompt／JSON Schema。
...
storage_v33.py                 v4.2 語義、盲解、決策、鎖定與 GitHub 後台
...
AI_DELIBERATION_MODEL = "openai/gpt-4.1-mini"
AI_TOP_K_CASES = 5
...
`AI_DELIBERATION_MODEL` 可指定較強模型專門負責第一階段語義盲解；省略時沿用 `AI_MODEL`。mini 模型較省額度，但語義輸出可能更模板化。一次完整推理會各呼叫盲解模型與決策模型一次。
...
規則即使生效，雙方 λ 仍受 ±25% 總上限約束；精確比分樣式的綜合倍率也限制在 0.75–1.25。劇本另以最高 46% 的情境分布與 Poisson 網格混合，不會偷偷改寫 λ。AI 盲解不會看到這些數值，避免先看到答案再補理由。詳見 [BLIND_DELIBERATION.md](docs/BLIND_DELIBERATION.md)、[HEXAGRAM_SCRIPT.md](docs/HEXAGRAM_SCRIPT.md) 與 [RULE_GOVERNANCE.md](docs/RULE_GOVERNANCE.md)。

## 評估

v4 系列只把以下案例計入正式盲測：

- 系統版本以 `4` 開頭；版本比較時另按 4.0、4.1、4.2 分層；
- 賽前狀態為 `已鎖定`；
- 之後才回填有效的 90 分鐘實際比分；
- 預測內容雜湊與版本仍完整。
...
- 第一階段 payload 不含比分、規則排名、足球 λ、實力分、量化門檻與相似案例賽果。
- 兩階段模型呼叫順序、盲解保存與第二階段中性候選順序。
- 單場規則權重為 0。
- 平衡候選池與 AI 方向保護。
- 同場案例排除。
- 舊 CSV 遷移。
...
2. 部署 v4.2 程式，但保留原 CSV、`reports/` 與 Streamlit Secrets。
3. 開啟 App；讀取時會在記憶體中補齊新欄位，不會刪除未知舊欄位。
4. 先確認舊案例仍可在賽後中心與成績頁顯示。
5. 用一場測試比賽建立 v4.2 鎖定版本，確認本地語義、AI盲解與第二階段決策分開顯示後再回填結果。
...
- [BLIND_DELIBERATION.md](docs/BLIND_DELIBERATION.md)：兩階段隔離、輸入契約與失敗回退。
...
- 每次兩階段 AI 推理會使用兩個模型請求；模型品質取決於所選 GitHub Models 模型，不能宣稱與本對話模型完全相同。
- AI 停用或呼叫失敗時仍有完整本地語義骨架，但它是可重現模板，不等同大型語言模型的開放推理。
