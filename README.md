# 梅花易數足球 AI 自主推理系統 v4.0

這是一個以 Streamlit、GitHub Contents 與 GitHub Models 建立的足球賽前研究系統。它把「足球先驗」與「梅花易數卦象」分成兩層，保存不可覆寫的賽前版本，再於賽後回填 90 分鐘結果，檢驗卦象修正與 AI 是否真的帶來增益。
...
4. 純足球先驗先建立雙方期望進球 λ。
5. 本卦、互卦、動爻、變卦與體用生剋沿同一條卦線解讀，只能對單方 λ 作 ±25% 有界修正。
6. 單場賽果產生的規則一律是 `hypothesis`，預測權重為 0；通過留出驗證後才能升級。
7. 每次不同的賽前預測會建立新版本；已回填的實際比分與校準內容不可被重新預測覆蓋。
8. 足球基線、卦象調整、AI 與最終排序分開評估。
...
- 足球基線 λ、卦象倍率、修正後 λ、勝平負機率與五球以上尾部機率。
...
storage_v33.py                 v4 資料遷移、鎖定、版本與 GitHub 後台
...
規則即使生效，雙方 λ 仍受 ±25% 總上限約束；精確比分樣式的綜合倍率也限制在 0.75–1.25。詳見 [RULE_GOVERNANCE.md](docs/RULE_GOVERNANCE.md)。
...
2. 部署 v4 程式，但保留原 CSV、`reports/` 與 Streamlit Secrets。
3. 開啟 App；讀取時會在記憶體中補齊新欄位，不會刪除未知舊欄位。
4. 先確認舊案例仍可在賽後中心與成績頁顯示。
5. 用一場測試比賽建立 v4 鎖定版本，再回填測試結果。
