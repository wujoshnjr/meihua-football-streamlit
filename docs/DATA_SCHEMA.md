...
- 時間：官方 `event_at`、自動 `freeze_at`、實際執行 `cast_at`、事件時區、完整農曆時間、干支年、日辰、月令、旬名、旬空及干支時辰。
...
同一次排卦結果重複儲存只確認既有列。重新按下排卦會取得新的 `cast_at` 稽核快照，因此即使文字與 `event_at` 相同，也會形成可獨立追溯的新排卦紀錄。任何時間都不參與文字取數；時間結構欄位只使用 `event_at`。
...
- `event_at`：官方排定開球時間；卦理唯一時間環境。
- `freeze_at`：固定為 `event_at − 6h`。
- `cast_at`：實際執行時間；只供稽核。
- `時間基準版本`：目前固定 `event-at-only-v1`。
- `樣本分類`：`CLEAN_BLIND`、`EXPOSED_BLIND` 或 `POSTMATCH_ANALYSIS`。
- `事件時間來源等級`、`事件時間來源網址`：保存 A／B／C 及證據。

既有「起卦國曆／農曆／時區、日辰、月令、旬空」欄位在新資料結構中均代表 `event_at` 環境；`建立時間` 與 `cast_at` 保留實際執行紀錄。

預測與賽果不加入排卦 CSV。它們的獨立 schema、指紋與評估規則見 `docs/FORECAST_EVALUATION.md`。
