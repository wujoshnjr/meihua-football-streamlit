...
st.success(
    "核心：奇門遁甲知識庫 × 梅花易數 ×《周易》64卦/384爻原典審查 ×《焦氏易林》4096轉卦 × deterministic 起局／起卦 × AI 解卦包。",
    icon="⚡",
)
...
st.info(
    ...
    "兩者的『完整』都指固定底本與資料矩陣，不代表所有歷代版本、標點與注家已全部校勘。"
)
...
3. 梅花會先核對《周易》本卦／互卦／變卦的卦辭、彖、象，並取出**真正動爻爻辭**與來源頁碼。
4. JARVIS 再抓本盤相關的梅花體用、旺衰、上下卦與足球 modern application 語義。
5. 以唯一的「本卦 → 最終變卦」查《焦氏易林》4096 catalog；易林補轉變情境，不重起一套卦。
6. 系統產生 `DIVINATION_PACKET_V2`，保留盤象、周易經文、動爻、易林林辭、來源、project heuristic、支持與反證。
7. 把 AI 解卦包交給 ChatGPT；最後綜合判讀由 AI 完成。

**原則：周易核文本 × 梅花定結構 × 易林補劇情 × ChatGPT 合參。** 任何足球語義都是 modern application，JARVIS 不把單一符號、爻辭或林辭寫死成勝率或固定比分。
"""
)
...
with right:
    with st.container(border=True):
        ...
        st.write(
            ...
            "核對《周易》卦辭／彖／象與真正動爻，再查唯一『本卦→變卦』易林林辭與情境語義。"
        )
...
st.caption("周易 64/384 是固定來源文本覆蓋；易林 heuristic 覆蓋只是語義檢索覆蓋。兩者都不是術數有效率或足球預測準確率。")
...
st.divider()
st.caption("JARVIS 負責盤、原典審查與知識；ChatGPT 負責解。古籍、注解、專案 heuristic 與足球 modern application 嚴格分層。")
