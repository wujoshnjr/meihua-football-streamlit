...
st.caption(
    "兩個模組保持獨立：QIYAO_RAW 只整理演數七要；RIQIMEN_QIYAO_EXPERIMENT 才額外保存日奇門 base。"
    "目前不自動把數宮或數主轉成足球比分。"
)

st.warning(
    "原典已能確定七要項目、數主落宮的重要性、日奇門60日『某宮起休』表與部分排法；"
    "但『遁至本時之星』完整飛遁算法與日奇門『穿宮數去』尚未完成 source reconstruction。"
)

with st.form("yuanling_yanshu_form"):
    question = st.text_area(
        "問題",
        value="依《元靈經》演數七要整理此事件之數術原始資料；保留未決項，不直接換算足球比分。",
        height=90,
    )
    ...
    with c2:
        event_time = st.time_input("事件所在地時間", value=time(20, 0), step=1, format="24h")
    with c3:
    ...
    with st.expander("研究輸入（只有在你已由原典/人工重建取得時才填）"):
        st.caption("這些欄位不是 JARVIS 猜出來的；未提供就維持 UNRESOLVED_BY_SOURCE_AUDIT。")
        r1, r2, r3 = st.columns(3)
        with r1:
            number_palace = st.selectbox("數宮", ["未提供", 1, 2, 3, 4, 5, 6, 7, 8, 9])
        with r2:
            chief_star = st.selectbox("數主星號（一白=1…九紫=9）", ["未提供", 1, 2, 3, 4, 5, 6, 7, 8, 9])
        with r3:
            chief_palace = st.selectbox("數主落宮", ["未提供", 1, 2, 3, 4, 5, 6, 7, 8, 9])
        ...
        with r6:
            daily_star = st.selectbox("直日星號", ["未提供", 1, 2, 3, 4, 5, 6, 7, 8, 9])

    submitted = st.form_submit_button("建立 YUANLING_YANSHU_PACKET_V1", type="primary", use_container_width=True)
...
if submitted:
    try:
        ...
        if audit["ambiguous"] and fold_mode == "AUTO_REJECT_AMBIGUOUS":
            raise EventLocalTimeError("此 local time 在 DST 回撥時出現兩次，請明確選 fold。")
        ...
        packet = build_yuanling_yanshu_packet(
            ...
            mode=mode,
            number_palace=None if number_palace == "未提供" else int(number_palace),
            number_chief_star_number=None if chief_star == "未提供" else int(chief_star),
            number_chief_landing_palace=None if chief_palace == "未提供" else int(chief_palace),
            flying_star=flying_star.strip() or None,
            entry_door=entry_door.strip() or None,
            daily_star_number=None if daily_star == "未提供" else int(daily_star),
        )
        st.session_state["stark_yuanling_packet"] = packet
        st.success("Yuanling packet 已建立；未解規則維持 unresolved，比分映射保持 disabled。")
...
if packet:
    ...
    rows = [
        {
            "七要": row["name"],
            "status": row["status"],
            "value": json.dumps(row["value"], ensure_ascii=False) if row["value"] is not None else "—",
            "note": row["note"],
        }
    ...
    with st.expander("元靈數術九星 registry（獨立於天蓬/天芮系）"):
        st.dataframe(qiyao["numeric_star_registry"]["stars"], hide_index=True, use_container_width=True)
    ...
    if packet["riqimen_base"]:
        ...
        b4.metric("起休宮", base["source_reconstructed"]["rest_door_start_palace"])
        with st.expander("地盤與 unresolved steps"):
            st.json(base)

    st.markdown("## 邊界")
    st.info(qiyao["boundary"])
    st.caption("raw numeric candidates = DISABLED_UNTIL_ALGORITHM_SOURCE_LOCK；score synthesis = deferred until blind-test protocol.")
