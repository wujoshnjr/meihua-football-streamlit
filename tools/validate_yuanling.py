...
def main() -> None:
    ...
    require(
        audit["source_policy"]["do_not_merge_adjacent_sections_without_explicit_text"] is True,
        "section separation rule missing",
    )
    require(
        audit["source_policy"]["do_not_reuse_shijia_star_registry_for_qiyao_by_default"] is True,
    ...
    require(
        table[0]["day_ganzhi"] == "甲子" and table[0]["rest_door_start_palace"] == 1,
        "甲子 table anchor mismatch",
    )
    require(
        table[-1]["day_ganzhi"] == "癸亥" and table[-1]["rest_door_start_palace"] == 4,
