...
def main() -> None:
    ...
    require(len(fixtures) == 4, "need exactly four source-derived method fixtures")
    ...
    for fixture in fixtures:
        ...
        fid = fixture["id"]
        require(board.dun == expected["dun"], f"{fid}: dun mismatch")
        require(board.yuan == expected["yuan"], f"{fid}: yuan mismatch")
        require(board.ju == expected["ju"], f"{fid}: ju mismatch")
        ...
        for number_text, stem in expected["earth"].items():
            require(board.palaces[int(number_text)].earth_stem == stem, f"{fid}: palace {number_text} earth stem mismatch")
        for number_text, door in expected["doors"].items():
            require(board.palaces[int(number_text)].door == door, f"{fid}: palace {number_text} door mismatch")
        for number_text, stars in expected["stars"].items():
            require(board.palaces[int(number_text)].stars == stars, f"{fid}: palace {number_text} stars mismatch")
        for number_text, stems in expected["heaven_stems"].items():
            require(board.palaces[int(number_text)].heaven_stems == stems, f"{fid}: palace {number_text} heaven stems mismatch")
    ...
    require({1, 4, 6, 9} <= seen_ju, "fixtures must preserve source-declared ju 1/4/6/9")
    require({"天遁", "地遁"} <= seen_source_patterns, "fixtures must cover Tian Dun and Di Dun")
    print("Qimen source golden validation passed: 4 source-derived examples / yang+yin / ju 1,4,6,9 / full core earth-star-door-heaven-stem expectations / deity layer excluded by method boundary")
