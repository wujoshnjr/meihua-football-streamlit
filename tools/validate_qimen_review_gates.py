...
def main() -> None:
    ...
    require(set(golden.get("fixture_classes", {})) == {"SOURCE_DERIVED_METHOD_GOLDEN", "END_TO_END_EXTERNAL_CHART", "INTERNAL_REGRESSION"}, "fixture classes incomplete")
    ...
    require(source_count == len(fixtures.get("fixtures", [])) == 4, "source-derived golden count must be four")
    ...
    require(end_to_end_count == 0, "end-to-end external chart count must remain explicitly zero in 10.2")
    require(golden["fixture_classes"]["SOURCE_DERIVED_METHOD_GOLDEN"]["counts_toward_full_end_to_end_engine_certification"] is False, "source-derived fixtures must not masquerade as end-to-end certification")
    ...
    require({"automatic_win_probability", "fixed_score_from_relation", "postmatch_backfill"} <= forbidden, "forbidden shortcuts incomplete")

    print(f"Qimen review gates: PASS | source-derived-golden={source_count} | issue62-golden={gate['issue_62_qimen_golden_complete']} | full-end-to-end-certified={gate['full_end_to_end_engine_certified']} | extended-static=378")
